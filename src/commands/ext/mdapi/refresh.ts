...
import { MdapiRefreshUtility } from '../../../scripts/mdapi-refresh-utility';

Messages.importMessagesDirectory(__dirname);

const messages = Messages.loadMessages('sfdx-ext', 'mdapi-refresh');
...
export default class Refresh extends SfdxCommand {
  ...
  public static examples = [
    `
    $ sfdx ext:source:refresh --targetusername user@example.com --apiversion 46.0 --ignorebackup --ignoremanaged --ignorenamespaces --manifestonly
    `,
    `
    $ sfdx ext:source:refresh --targetusername user@example.com
    `
  ];
  ...
  public async run(): Promise<any> {
    ...
    console.log("-----------------------------");
    console.log("sfdx ext:mdapi:refresh");
    console.log("-----------------------------");
